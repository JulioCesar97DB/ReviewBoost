...
export function GoogleIntegrationCard() {
			...
			if (isMockMode()) {
				...
				if (status.connected && status.business) {
					setBusiness({
						...
						google_connected_at: status.business.google_connected_at,
						last_review_sync_at: new Date().toISOString(),
						total_reviews: MOCK_STATS.totalReviews,
						avg_rating: MOCK_STATS.averageRating,
					});
				} else {
					setBusiness({
	...
	const handleConnect = async () => {
		if (isMockMode()) {
			setConnecting(true);
			try {
				const result = await MockGoogleConnectionService.connect();
				if (result.success) {
					setBusiness({
						id: result.business.id,
						name: result.business.name,
						google_place_id: result.business.google_place_id,
						google_connected_at: result.business.google_connected_at,
						last_review_sync_at: null,
						total_reviews: MOCK_STATS.totalReviews,
						avg_rating: MOCK_STATS.averageRating,
					});
	...
	const handleDisconnect = async () => {
		...
		try {
			if (isMockMode()) {
				await MockGoogleConnectionService.disconnect();
				setBusiness({
					...business,
					google_place_id: null,
					google_connected_at: null,
	...
	const handleSync = async () => {
		...
		try {
			if (isMockMode()) {
				const result = await MockReviewsService.syncFromGoogle();
				setSyncResult({
					success: true,
					message: `Synced ${result.synced} reviews (${result.new} new)`,
					stats: {
						totalReviews: MOCK_STATS.totalReviews,
						avgRating: MOCK_STATS.averageRating,
						reviewsThisMonth: 8,
	...
	const isConnected =
		business?.google_place_id && business?.google_connected_at;
